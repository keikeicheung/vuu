...
class BasketCreateTest extends VuuServerTestCase {
  ...
  Feature("Create Basket Test Case") {
    Scenario("Basket should contain its default constituents when user created a basket") {
      ...
      withVuuServer(PriceModule(), BasketModule()) {
        vuuServer =>
          ...
          basketProvider.tick(".TEST", Map(BasketColumnNames.Id -> ".TEST",
            BasketColumnNames.Name -> ".TEST", BasketColumnNames.NotionalValue -> 8000,
            BasketColumnNames.NotionalValueUsd -> 1000))

          val viewport = vuuServer.createViewPort(BasketModule.NAME, BasketTable)

          vuuServer.runOnce()

          assertVpEq(combineQsForVp(viewport)) {
            Table(
              (BasketColumnNames.Id,
                BasketColumnNames.Name, BasketColumnNames.NotionalValue,
                BasketColumnNames.NotionalValueUsd),
              (".TEST", ".TEST", 8000, 1000)
            )
          }
          ...
          val viewportBasketConstituent = vuuServer.createViewPort(BasketModule.NAME, BasketConstituentTable)
...
//          basketTradingService.editCellAction().func("chris-001", BT.Units, 100.asInstanceOf[Object], viewportBasketTrading, vuuServer.session)
          ...
          assertVpEq(combineQsForVp(viewportBasketConstituent)) {
            Table(
              ("ricBasketId", "ric"),
              (".FTSE", ".FTSE")
            )
